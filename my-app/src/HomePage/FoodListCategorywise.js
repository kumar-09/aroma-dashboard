...
const FoodListCategorywise = ({ Foodlist, subtractOne, addOne, MainClass, cart, ImgClass, ItemInfo, ItemPriceInfo, AddBtn, ItemName }) => {
  ...
  return (
    <div id={Foodlist[0] + 'list'}>
      ...
      {Foodlist[1].map((Food) => (
        <ItemComponent Food={Food} key={Food.food_id} subtractOne={subtractOne} addOne={addOne} ItemInfo={ItemInfo} ItemPriceInfo={ItemPriceInfo} AddBtn={AddBtn} ItemName={ItemName} ImgClass={ImgClass} MainClass={MainClass} quantity={((cart.find((eatable => eatable.food_id === Food.food_id))) ?? { quantity: 0 }).quantity} />
      ))}
    </div>
  )
}
